...
type FreeQiYunProxy struct {
	page int
	url  string
	reg  string
}

func (fp *FreeQiYunProxy) Name() string {
	return "齐云代理"
}

var FreeQiYun = FreeQiYunProxy{
	page: 20,
	url:  "https://proxy.ip3366.net/free/?action=china&page=",
	reg:  `<td data-title="IP">([\d.]+)</td>\s*<td data-title="PORT">(\d+)</td>`,
}

func (fp *FreeQiYunProxy) Fetch() ([]string, error) {
...
func (fp *FreeQiYunProxy) fetchPage(page int) ([]string, error) {
	client := req.C()
